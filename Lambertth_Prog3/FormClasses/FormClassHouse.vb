...
Public Class FormClassHouse
   ...
   Public Sub New()
      ...
      _house = New CHouse(51423, "Platteville")
      _house.remove(_house)
   End Sub
   ...
   ' Handles the HouseListUpdated event. Shows a messagebox if the price changed.
   Protected Sub listUpdated() Handles _house.HouseListUpdated
      Dim flag As Boolean = False
      Try
         _frmlist.updates()
         For index As Integer = 0 To House.TotalCount - 1
            If (_house.ID = House.HouseByIndex(index).ID) Then
               flag = True
            End If
         Next
         If (flag = True) Then
            If Not (_house.Price = txtPrice.Text) Then
               DisplayHouse(_house)
               MessageBox.Show("Price has changed!")
            End If
         Else
            Throw New Exception
         End If
   ...
   ' Sets the listform parent to the MDIform
